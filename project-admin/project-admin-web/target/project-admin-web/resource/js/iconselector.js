...
//图标
var jiconlist, winicons, currentComboBox,currentgrid;
$(function ()
{
...
}).live('click', function ()
{
    ...
    if(src.indexOf(basepath)>=0){
    	src = src.substring(src.indexOf(basepath)+basepath.length,src.length);
    }
    src = src.replace(/^([\.\/]+)/, '');
    if (currentComboBox)
    {
...
function f_openIconsWin()
{
    ...
    {
        LG.ajax({
        	url: basepath+'SystemWeb/getIcons.do',
            loading: '正在加载图标中...',
            ...
            success: function (data)
            {
                ...
                {
                    var src = data[i];
                    var reg = /(resource\\icons)(.+)/;
                    var match = reg.exec(src);
                    if (!match) continue;
                    var s = basepath+"resource/icons" + match[2].replace(/\\/g, '/');

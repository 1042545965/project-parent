...
//几个布局的对象
        ...
        $(document).ready(function ()
        {
            ...
            //布局初始化 
            //layout
            layout = $("#mainbody").ligerLayout({ 	leftWidth : 150,
				height : '100%',
				heightDiff : -10,
				space : 4,
           ...
           // alert(basepath+'systemMenu/getMenus.do');
            $.getJSON(basepath+'systemMenu/getMenus.do?Math.random', function (menus)
            {
                $(menus).each(function (i, menu)
                {
                    ...
                    var topitem=$('<li><a >'+ menu.menuname + '</a><ul></ul></li>');
                    $(menu.children).each(function (j, submenu)
                    {
                    	var url=basepath+submenu.menuurl;
                    	//给url附加menuno
                        if (url.indexOf('?') > -1) url += "&";
                        else url += "?";
                        
                        url += "MenuNo=" + submenu.menuno;
                       //left导航
                        ...
                        //top导航
//                        var topsubitem = $('<li><a ><img height="16px" width="16px"/>'+submenu.menuname+'</a></li>');
//                        topsubitem.attr({
//                            menuno: submenu.menuno
//                        });
//                        $("img", topsubitem).attr("src", basepath+submenu.icon || basepath+submenu.icon);
//                        $("ul:first", topitem).append(topsubitem);
                    });

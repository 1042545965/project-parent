...
$.ligerui.controls.Grid.prototype.getParemter = function(){
	var g = this, p = this.options;
	return p.parms;
}
...
$.ligerui.controls.Grid.prototype.showFilter = function ()
{
    ...
    g.winfilter = $.ligerDialog.open({
        ...
        buttons: [
{ text: '确定', onclick: function (item, dialog) { loadFilterData(g); dialog.hide(); } },
{ text: '取消', onclick: function (item, dialog) { dialog.hide(); } }
    ...
    function loadFilterData(grid)
    {
        ...
        {
            g.set('parms', { where: JSON2.stringify(data),where2:JSON2.stringify(datarule) });
        } else
        {
            g.set('parms', {where2:JSON2.stringify(datarule)});
        }
        g.loadData();
    }
};
...
//使下拉框支持 在弹出窗口在选择
$.ligerui.controls.ComboBox.prototype.openMapSelect = function (options,value)
{
    this._v = value;
    var g = this, p = this.options;
    options = $.extend({
  ...
  //三个 ligerui 对象
    ...
    function show()
    {
        ...
        {
            var panle = $("<div id='tempdata987654321'></div>");
            $("body").append(panle);
            //dialog
            win = $.ligerDialog.open({
                ...
                isHidden:false,
                left: options.left,
                url: basepath+'resource/js/Map.html?coord='+ g._v,
                buttons: [
    ...
    function toSelect()
    {
       var tempdata = $('#tempdata987654321').html();
       g._changeValue(tempdata, tempdata);
    }
 }
//使下拉框支持 在弹出窗口在选择

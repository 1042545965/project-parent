...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setValueById:function(treeid,id){
        	var _tree=$("#"+treeid).find('li');
        	$.each(_tree, function (i, m) {
        		if(m.id==id){ 
        			$(m).find('.l-checkbox').removeClass('l-checkbox-unchecked').addClass('l-checkbox-checked');
        			if($(m).siblings().find('.l-checkbox-unchecked').size()==0){
        				$(m).parents('li').find('.l-checkbox').removeClass('l-checkbox-unchecked').addClass('l-checkbox-checked');
        			}
        		}
			});

...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            $("#boxSelect"+g.textFieldID).remove();
            //下拉框
            g.selectBox = $('<div class="l-box-select" id="boxSelect'+g.textFieldID+'" style="display:none"><div class="l-box-select-inner"><table cellpadding="0" cellspacing="0" border="0" class="l-box-select-table"></table></div></div>');
            ...
            //添加个包裹，
            g.textwrapper = g.wrapper.wrap('<div class="l-text-wrapper" id="textwrapper_'+g.textFieldID+'"></div>').parent();
            ...
            $(document).click(function(event){ 
            	 if (p.hideOnLoseFocus && g.selectBox.is(":visible") && !g.boxToggling && !g.resizing 
            			 && $(event.target).parents("#textwrapper_"+g.textFieldID).length==0 && $(event.target).parents("#boxSelect"+g.textFieldID).length==0)
                 {
                     g._toggleSelectBox(true);
                 }
            }) ;
...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            var dateeditorHTML = "";
            dateeditorHTML += "<div class='l-box-dateeditor'  id='dateeditor_"+p.fieldname+"' style='display:none'>";
            ...
            *************/
            ...
            $(document).click(function(event){            	
              	 if (g.dateeditor.is(":visible") && !g.editorToggling 
              			 && $(event.target).parents("#dateeditor_"+p.fieldname).length==0)
                   {
                       g.toggleDateEditor(true);
                   }      	
              }) ;
...
(function ($)
{
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        }, field.options, field.editor, {fieldname:field.name},options);
            ...
            else
            {
                ...
                switch (ltype)
                {
                    ...
                    case "popup":
                        ...
                        if(field.validate)
                       	 	textInput.attr("validate", JSON2.stringify(field.validate));
                        if (p.appendID)
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _buliderControl: function (field, fieldIndex)
        {
            ...
            if(field.cssClass){
            	if(type == "textarea"){
            		out.push('class="' + field.cssClass + '  l-textarea" ');
            	}else{
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:20px' maxlength='6' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadData: function (loadDataParm)
        {
            ...
            g.loading = false;
            $(document).find(".l-grid-hd-row").each(function(){
            	if($(this).attr("tagName")=="TR" && $(this).hasClass("l-checked")){
            		$(this).removeClass("l-checked");
            	}         	
            });
        },
